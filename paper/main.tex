...
CI/CD pipelines can fail for many different reasons. A failure may be caused by a recent code change, a test or flaky test, a dependency problem, or a CI/environment configuration issue. In some cases, the failure can also be related to infrastructure, an external service, or other anomalous events. The main problem is that the actual cause is not directly observable when the failure first occurs. The agent therefore has to reason about multiple possible causes and decide what to investigate next.
...
\begin{itemize}
    \item A CI failure diagnosis formulation in which the root cause is represented as a hidden state spanning five distinct categories with an associated probability.
...
% ============================================================
% RELATED WORK AND USER DISCUSSIONS
% ============================================================
...
The V1 quantitative experiment uses five hidden states:
\begin{enumerate}
    ...
    \item \textbf{Something else}: the failure is caused by residual, out-of-distribution, infrastructure, or external service issues.
\end{enumerate}
...
Before considering evidence from the current case, the agent starts with an empirical prior estimated from the historical labeled dataset. After applying the V1 hidden-state mapping across all five states, 305 of the 375 records were retained for quantitative probability estimation.

The resulting empirical prior was:

\begin{table}[t]
\centering
\caption{Empirical prior distribution across the five hidden states ($\epsilon = 0.02$).}
\label{tab:prior}
\begin{tabular}{lr}
\toprule
...
\begin{table}[t]
\centering
\caption{Likelihood of observing a Build failure for each hidden state.}
...
\[
P(H\mid E,o,a) = \frac{P(o\mid H,E,a)P(H\mid E)}{P(o\mid E,a)}.
\]
The denominator normalizes the probabilities:
\[
P(o\mid E,a) = \sum_{h} P(o\mid h,E,a)P(h\mid E).
\]
...
\[
H(H\mid E) = -\sum_h P(h\mid E)\log P(h\mid E).
\]
...
\[
EIG(a) = \sum_o P(o\mid E,a)IG(o).
\]
The probability of an outcome is obtained by marginalizing over the five possible hidden states:
\[
P(o\mid E,a) = \sum_h P(o\mid h,E,a)P(h\mid E).
\]
...
\subsection{Decision Policy and Stopping Threshold}

In the V1 evaluation, the agent stops diagnostic actions and reports root cause $H_i$ as soon as its posterior belief satisfies $P(H_i \mid E) \geq 0.90$. If the available candidate actions are exhausted without any hidden state reaching this threshold, the agent escalates the case for human review.
...
\begin{figure}[t]
    ...
    \caption{Bayesian and EIG-based action-selection process across the five hidden states. The agent maintains a posterior belief, evaluates possible action outcomes, computes information gain and expected information gain, ranks actions by EIG/cost, observes the selected action's outcome, and updates its posterior.}
    \label{fig:eig}
\end{figure}
...
The V1 agent uses a strict hidden-state mapping. Categories mapped clearly to the five quantitative hidden states (Code, Test, Dependency, CI/Config, and Something else) are included, yielding 305 usable records out of the original 375.
...
Case 9 reached 88.3\% probability for CI/Config but remained below the 90\% reporting threshold. In this situation, the diagnostic evidence was moving the belief towards the correct root cause, but the static stopping criterion prevented the agent from reporting it.
...
% ============================================================
% LIMITATIONS, ETHICS, AND HUMAN CONTROL
% ============================================================
...
A third limitation is the heuristic reporting threshold. The static 90\% threshold was selected using intuition rather than being calibrated from historical decision outcomes.
...
At the same time, the failure analysis shows that the current system is limited by the quality of the simulated action outcome models and the static decision rule. To address these limitations, future iterations will explore the following architectural extensions:

\subsection{Jensen-Shannon Divergence Policy}
While Shannon Entropy and EIG measure total uncertainty reduction, they do not directly evaluate how effectively an action differentiates between specific competing hypotheses. Future scope includes replacing or augmenting the EIG/cost policy with a selection metric based on Jensen-Shannon Divergence (JSD). For a set of candidate posterior belief distributions $P_o = P(H \mid E, o, a)$ resulting from possible outcomes $o \in \mathcal{O}$, the JSD is defined as:
\[
\text{JSD}(P_{o_1}, P_{o_2}, \dots) = H\left(\sum_{o} w_o P_o\right) - \sum_{o} w_o H(P_o)
\]
where $w_o = P(o \mid E, a)$. Selecting actions that maximize $\frac{\text{JSD}(a)}{\text{Cost}(a)}$ ensures that the chosen diagnostic step maximizes the statistical distance between alternative state hypotheses, preventing the agent from selecting low-cost actions that provide information without resolving ambiguity between top candidate causes.

\subsection{State-Specific Optimal Stopping Thresholds}
Rather than relying on an arbitrary constant threshold (e.g., 90\%), future work will implement state-specific stopping thresholds derived from formal Bayesian decision theory. By defining a cost matrix representing the cost of a false positive ($C_{\text{FP}, i}$) and false negative ($C_{\text{FN}, i}$) for each hidden state $H_i$, the optimal decision threshold $p^*_i$ can be formally derived as:
\[
p^*_i = \frac{C_{\text{FP}, i}}{C_{\text{FP}, i} + C_{\text{FN}, i}}
\]
Table~\ref{tab:thresholds} outlines proposed state-specific thresholds based on domain risk profiles.

\begin{table}[t]
\centering
\caption{Proposed state-specific stopping thresholds $p^*_i$ based on domain risk.}
...
Under this framework, the agent will stop diagnostic actions and report root cause $H_i$ as soon as its posterior belief satisfies $P(H_i \mid E) \geq p^*_i$, balancing risk dynamics far more effectively across high-risk changes (e.g., Code) versus lower-risk fixes (e.g., Dependency).
...
\textbf{Problem selection:} Panduru Grishm Siddharth.\\
\textbf{Public discussions:} Panduru Grishm Siddharth.\\
\textbf{Agent design:} Panduru Grishm Siddharth.\\
\textbf{Software development:} Panduru Grishm Siddharth.\\
\textbf{Tests:} Panduru Grishm Siddharth.\\
\textbf{Citation checks:} Panduru Grishm Siddharth.\\
\textbf{Preprint sections:} Panduru Grishm Siddharth.\\
\textbf{PDF checks:} Panduru Grishm Siddharth.
...
The code and project materials are available at: \url{https://github.com/Proudprogamer/CI-Agent}
